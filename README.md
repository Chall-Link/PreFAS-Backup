...
### PreFAS特徴
- 姉妹ソフトChall-Link「VSSDetector」を併用することで、**VSS** スナップショットを使って、バックアップ処理中でも元フォルダで通常作業を継続できます
- AES-256で暗号化された7-Zipを作成します
- テラバイトレベルの大容量ドライブ、大容量フォルダもアーカイブ可能です（[7-Zip公式サイト](https://www.7-zip.org/7z.html) より）
- カスタマイズ設定はスクリプトに直接書込みますので、何度も実行する場合、前回の設定を覚えて置く必要は無く、同じスクリプトをコピーするだけで、いつでも自分好みの設定を繰り返し利用可能です
- 7-ZipのGUIからはアクセスできない、全ての7-Zip制御パラメータを利用できます
- 作成されたバックアップファイルは通常ファイルとして、エクスプローラーで自由にコピー・移動などが可能です
