...
この課題解決のために考案したファイル保護のアイデアは以下の通りです。  

- **対象**: 大切なファイルが含まれるテラバイトレベルの大容量ドライブや大容量フォルダ  
- **手段**: 7-Zipで次の条件でアーカイブ化  
  - 25GBに分割  
  - AES-256暗号化  
- **運用**:  
  - 光ディスクに保存して遠隔地に保存（M-Discなら100年レベルの耐久性。一般的なDVD・Blu-rayの寿命は最大20年程度と言われている）  
  - クラウドにアップロード保存  

このアイデアにより、20～30桁のランダムな英数記号パスワードでPreFAS Backupでアーカイブを作成して、パスワードさえ自分で保管しておけば、遠隔地に保存しながらファイルの中身を他人に見られることもなく、大切なファイルを長期安全に分散保存し、災害による紛失リスクを限りなくゼロに近づける事が可能となります。  
**AES-256暗号化と20～30桁のランダムな英数記号パスワードの組み合わせ**は、現代の技術ではほぼ解読不可能と言われております。  

これが、Chall-Link「PreFAS Backup」の制作コンセプトです。  

&emsp; 

### Chall-Link「PreFAS Backup」／ シャルリンク「プリファス バックアップ」
`Chall-Link「PreFAS Backup」は、このアイデアを具現化したスクリプトです`  
PreFAS Backupは、このアイデアを最適に実行するために具現化された7-Zip機能拡張スクリプトです。
