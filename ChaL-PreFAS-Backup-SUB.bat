...
:: WindowsのVSS（スナップショット）機能を利用する事で問題を回避できます。
:: スナップショットは復元ポイント作成時に生成される「過去の時点での静的なドライブ状態」で、
:: 内容が変化しないためバックアップ元に指定すれば編集作業を継続できます。
:: スナップショット利用には専用パス（VSSパス）が必要で、姉妹ソフト VSSDetector 
:: https://chall-link.github.io/VSSDetector/ で取得可能です。
:: 詳細は VSSDetector の説明書をご確認ください。
